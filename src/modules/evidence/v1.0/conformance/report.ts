/**
 * Report rendering for the Evidence conformance runner. Reuses
 * `renderCheckLines` from the core conformance report — mirrors
 * `../../../answer/v1.0/conformance/report.ts`.
 */
import { renderCheckLines, type TextReportOptions } from "../../../../conformance/report.js";
import type { EvidenceConformanceReport, CheckResult, CheckStatus } from "./types.js";

export function renderEvidenceTextReport(report: EvidenceConformanceReport, options: TextReportOptions = {}): string {
  const color = options.color ?? false;
  const verbose = options.verbose ?? true;
  const lines: string[] = [
    `Evidence v${report.module.version} conformance${report.base_url ? ` — ${report.base_url}` : ""}`,
  ...
  lines.push("", renderEvidenceSummary(report));
  return lines.join("\n");
}

export function renderEvidenceSummary(report: EvidenceConformanceReport): string {
...
export function renderEvidenceJsonReport(report: EvidenceConformanceReport): string {
...
export interface EvidenceJUnitReportOptions {
  failOnWarning?: boolean;
}

export function renderEvidenceJUnitReport(report: EvidenceConformanceReport, options: EvidenceJUnitReportOptions = {}): string {
  ...
  lines.push(
    `<testsuite name="${xmlEscape(`aadp-evidence-conformance${report.base_url ? ` ${report.base_url}` : ""}`)}" ` +
  ...
  for (const check of report.checks) {
    ...
    const attrs = `classname="${xmlEscape(`aadp.evidence.${check.group}`)}" name="${xmlEscape(`${check.id} ${check.title}`)}" time="${timeS}"`;
    if (isFailure(check)) {
...
/** Exit code contract, mirroring the core/Relations/Answer runners': `0` conformant, `1` a check failed, `4` inconclusive. */
export function evidenceExitCodeFor(report: EvidenceConformanceReport): 0 | 1 | 4 {
