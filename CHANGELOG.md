...
> **Release gate: OPEN.** ADR-0011 was Accepted on 2026-08-12 and Phases 1-5 are complete and green, but the Phase 6 neutral interoperability runs have **not** been performed — no data set has been identified. This release MUST NOT be tagged until both runs exist and are recorded; see `docs/records/implementation-record-v1.5.0.md`.

### Added

- Added cross-module graph traversal (`ail-aadp/traversal/v1.0`, `src/traversal/v1.0/`): `traverseGraphV1()` streams the graph reachable from a root entity or entity URL, and `collectGraphV1()` is a drain of that same iterator rather than a second traversal algorithm. Cross-module traversal is a **client-side capability, not a wire contract** — `1.5.0` publishes no specification, and ADR-0011 is the binding source.
- Added a traversal adapter registry keyed by the exact triple `{moduleId, moduleVersion, extensionField}`, independent of the released `src/module-registry` (which is unchanged). Registering a *different* adapter under a taken key throws and re-registering the identical reference is a no-op, so a walk's result can never depend on module import order. `options.adapters` **replaces** the global registry for one call instead of merging with it, and `options.capabilities` is an exact `{moduleId, version}` allowlist whose exclusions look exactly like an unimplemented module.
- Added the three built-in adapters for Relations/Answer/Evidence `1.0`, registered only on an explicit `registerBuiltinTraversalAdapters()` call or through `options.adapters` — importing the subpath registers nothing. Each validates with the module's own RELEASED validator, so traversal never accepts a payload a standalone module client would reject, and both adapter steps are pure: no fetch, no budget charge, no clock.
- Added per-extension outcomes: a node carries one expansion record per `x_*` field, so an unsupported or malformed extension stops only its own adapter and every other adapter on that entity still plans its edges. Extensions are processed in extension-field **name** order (code point), never JSON property order, so two entities that serialize the same extensions differently produce the same event sequence.
- Added the edge matrix as normative for the traversal service: `relations.item`, `relations.collection` (opt-in `followCollections`, default `false`, paged by the released `iterateRelationCollection` and bounded by the budget alone — there is **no** page limit anywhere in this API), `answer.related_entity`, `answer.source_target` (opt-in `includeGeneratedSummarySources`, default `false`) and `evidence.evidence_ref` (leaf). An evidence entity's `source.url`/`publisher.url` are never fetched.
- Added three separate outcome vocabularies at three scopes: the RELEASED `AnswerTargetResolutionStatus` for nodes (no sixth value), `ExtensionExpansionOutcomeV1` for extensions, and `EdgeExpansionOutcomeV1` for edge occurrences. Every planned edge produces an occurrence result, **including** edges that are never fetched — a blocked edge is emitted at its scheduled position, carries no resolution `status` (nothing was attempted) and still counts in `summary.edges`.
- Added walk-local expansion state: `expandedKeys` and the ancestor chain live in the walk, never on the budget, so a second walk on the same budget expands the same nodes again from the canonical cache rather than silently skipping their outgoing edges. A true cycle (target on the occurrence's own ancestor path) is reported as `cycle`; a diamond re-entry is `already-expanded`, which is a different topology and is labelled as such.
- Added deterministic ordering independent of completion timing: a total schedule key `(depth, parentDiscoveryIndex, edgeGroupRank, edgeIndex)` decides emission order while several fetches may be in flight. Fetch concurrency is an internal constant (`4`) and is deliberately **not** public API at `1.0`.
- Added a bounded event buffer (`maxBufferedEvents`, default 256) with real backpressure, so a slow consumer slows the walk instead of accumulating the whole graph. There is no total-event limit, and therefore no `max-events` stop reason. Exactly one terminal `complete` event is emitted — always, including on budget exhaustion and abort — except after an early consumer `return`, which runs cleanup and declares no outcome.
- `summary.stopReason` and `summary.partial` answer different questions and are independent (ADR-0011 §9): the first says how the scheduler finished, the second whether the graph is missing anything. A recoverable branch failure — a `relations.collection` that could not be paged — yields `stopReason: "exhausted"` with `partial: true`, so a graph missing a whole branch is never reported as complete. The three-value `stopReason` vocabulary is unchanged; no released enum grew.
- `summary.requests` counts the logical canonical-target resolutions a walk **started** — not HTTP attempts. A cache hit, a join onto a resolution already in flight, a collection page, and a retry or redirect hop all leave it unchanged; for physical attempts, read the caller-owned `budget.requestsMade`. The two counters answer different questions and neither is derivable from the other (ADR-0011 §9).
- Added budget borrowing: the caller owns a `RelationsTraversalBudgetState`, traversal creates no child budget, raises no default and adds no seventh dimension. Resolution goes exclusively through the internal shared canonical-resolution layer keyed by that budget, so fan-in charges one node and fetches once, and the released `1.3.1` resolution-context binding still fails closed before any replay, charge or request.
- Added the conformance profile `aadp:graph-traversal@1.0` (`runGraphTraversalConformance`, `GRAPH_TRAVERSAL_CHECKS`) with 26 stable check IDs, reusing the core `CheckResult`/`ConformanceSummary` shapes rather than introducing a fourth report format. The report is `profile`-scoped, not `module`-scoped. The profile certifies a traversal *implementation* against ADR-0011, so it runs against an in-package fixture matrix and makes no request — a consumer can run it offline in their own CI from an install.
- Added the package export path `./traversal/v1.0`. It is the single public path: traversal is re-exported from neither the package root nor any module subpath, and the shared canonical-resolution layer is exported nowhere — both asserted against the packed tarball.
...
- `files` now excludes `node_modules` at any depth. npm's built-in "never pack `node_modules`" rule does not apply inside a directory the `files` allow-list names explicitly, and this package lists `examples` — so a maintainer who had installed the reference server example's own dependencies (which running it requires) would have published a tarball of 1,294 files / 5.2 MB containing a recursive copy of `ail-aadp` and its whole dependency tree, with `npm pack` exiting `0` and CI, which has no such directory, staying green. No released version shipped this way (`1.3.1` is 206 files); the defect was caught pre-tag by the `1.4.0` release review. `tests/package/tarball-contents.test.ts` now asserts the real `npm pack` output — no `node_modules` entry at any depth, the example source still present, nothing outside the package root.
