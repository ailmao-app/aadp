...
- Added the conformance profile `aadp:graph-traversal@1.0` (`runGraphTraversalConformance`, `GRAPH_TRAVERSAL_CHECKS`) with 25 stable check IDs, reusing the core `CheckResult`/`ConformanceSummary` shapes rather than introducing a fourth report format. The report is `profile`-scoped, not `module`-scoped. The profile certifies a traversal *implementation* against ADR-0011, so it runs against an in-package fixture matrix and makes no request — a consumer can run it offline in their own CI from an install.
