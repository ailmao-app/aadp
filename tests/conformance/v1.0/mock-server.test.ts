...
/**
 * Regression tests specific to this repo's own v1.0 reference/mock server
 * fixture dataset: type "example" (5 items, paginated at page size 2) and
 * type "note" (3 items, deliberately served unpaginated with no `cursor`
 * field). These assertions are tied to `mock-server.ts`'s hardcoded
 * datasets and MUST NOT be asserted by `conformance.test.ts`, which needs
 * to run unmodified against arbitrary external v1.0 servers.
 */
...
it("mock note sitemap is unpaginated: single page, no cursor field", async () => {
  ...
  const page = await fetchSitemap(sitemapUrl, null, PERMISSIVE);
  expect(page.items.length).toBe(3);
  expect(page.cursor).toBeUndefined();

  const allIds: string[] = [];
  for await (const item of iterateSitemap(sitemapUrl, PERMISSIVE)) allIds.push(item.id);
...
it("mock sitemap paginates 5 items into 3 pages of page size 2", async () => {
  ...
  const page1 = await fetchSitemap(sitemapUrl, null, PERMISSIVE);
...
describe("manifest surfaces the full v1.0 envelope", () => {
  it("modules, resources, interfaces and security_schemes are populated and mutually consistent", async () => {
    const manifest = await discover(server.baseUrl, PERMISSIVE);
    expect(manifest.modules).toHaveLength(1);
    expect(manifest.resources?.map((r) => r.type).sort()).toEqual(["example", "note"]);
    expect(manifest.interfaces).toHaveLength(1);
    expect(manifest.security_schemes?.guest).toEqual({ type: "none" });
  });
});
